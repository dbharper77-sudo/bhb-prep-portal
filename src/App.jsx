  ...
  return (
    ...
    <div className="page-body">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div className="card"><div className="card-title">Recent Items</div>{liquidationStock.length === 0 ? <div style={{ color: "var(--text-muted)", marginTop: 12 }}>No items yet.</div> : <div style={{ marginTop: 12 }}>{liquidationStock.slice(0, 5).map(s => { const c = calculatePayout(s); return <div key={s.id} style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}><div><div style={{ fontWeight: 600 }}>{s.product_name}</div><div style={{ fontSize: 12, color: "var(--text-muted)" }}>{s.asin}</div></div><div style={{ textAlign: "right" }}>{s.sale_price ? <><div className="mono" style={{ fontWeight: 700, color: "var(--green)" }}>£{c.payout.toFixed(2)}</div><div style={{ fontSize: 11, color: "var(--text-muted)" }}>{s.paid ? "Paid" : "Pending"}</div></> : <span className="badge badge-pending">Awaiting</span>}</div></div>; })}</div>}</div>
      </div>
    </div></>
  );
}
...
function AdminLiquidationPage({ token, showToast }) {
  ...
  return (
    ...
    <div className="page-body">
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}><div className="table-wrap"><table style={{ minWidth: 1000 }}>
        <thead><tr>{!selectedClient && <th>Client</th>}<th>Product</th><th>LPN</th><th>Condition</th><th>Listed</th><th>Sale £</th><th>Fees</th><th>Payout</th><th>Payout Date</th><th>Paid</th><th></th></tr></thead>
