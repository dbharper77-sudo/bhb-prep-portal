...
async function lookupAsinTitle(asin) {
  try {
    ...
    const data = await res.json();
    const match = data.contents && data.contents.match(/id="productTitle"[^>]*>\s*([^<]+)\s*</);
      ...
      {activeTab === "transit" && <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        ...
        <div className="table-wrap"><table style={{ width: "100%", tableLayout: "fixed" }}>
          ...
          <tbody>{transitItems.map(item => {
            const isEdit = editingId === item.id, data = isEdit ? editData : item;
            return <tr key={item.id} className={isEdit ? "edit-row" : ""}>
              <td style={{ fontWeight: 600 }}>{isEdit ? <div style={{ display:"flex", flexDirection:"column", gap:4 }}><input className="inline-input" style={{ width: 160 }} placeholder="Product name" value={data.product_name} onChange={e => setEditData({ ...editData, product_name: e.target.value })} /><div style={{display:"flex",gap:4,alignItems:"center"}}><input className="inline-input" style={{ width: 120 }} placeholder="ASIN" value={data.asin} onChange={e => setEditData({ ...editData, asin: e.target.value })} /><button type="button" style={{padding:"3px 8px",background:"var(--orange)",color:"#000",border:"none",borderRadius:5,fontSize:11,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}} onClick={() => { if (data.asin) window.open("https://www.amazon.co.uk/dp/" + data.asin, "_blank"); }}>🔍 Amazon</button></div></div> : <div>{item.product_name}<div style={{ fontSize: 11, color: "var(--text-muted)" }}>{item.asin}</div></div>}</td>
              <td>{isEdit ? <input className="inline-input" style={{ width: 80 }} value={data.lpn_number} onChange={e => setEditData({ ...editData, lpn_number: e.target.value })} /> : <span className="mono" style={{ fontSize: 12 }}>{item.lpn_number || "—"}</span>}</td>
              <td>{isEdit ? <input type="number" min="1" className="inline-input" style={{ width: 55 }} value={data.quantity} onChange={e => setEditData({ ...editData, quantity: e.target.value })} /> : <span className="mono">{item.quantity || 1}</span>}</td>
