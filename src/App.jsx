...
function AdminClientsPage({ clients, parcels, shipments, liquidation, onSelectClient, loading, token, onRefresh, showToast }) {
  ...
  const liqCount = sortedClients.filter(c => !archivedIds.includes(c.id) && (clientTypes[c.id] || c.client_type || "prep") === "liquidation").length;
  return (
    ...
    <div className="page-body">
      <div style={{ display: "flex", gap: 4, marginBottom: 16 }}>
        {[["all", `All (${allCount})`], ["prep", `Prep (${prepCount})`], ["deals", `Deal Sheet (${dealsCount})`], ["liquidation", `Liquidation (${liqCount})`]].map(([val, label]) => (
