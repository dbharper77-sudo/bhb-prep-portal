      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}><div className="table-wrap"><table style={{ minWidth: 1100 }}>
        ...
        <tbody>{clientItems.map(item => {
          ...
          return <tr key={item.id} className={isEdit ? "edit-row" : ""}>
            ...
            <td>{isEdit ? <input type="date" className="inline-input" style={{ width: 120 }} value={data.date_sold} onChange={e => setEditData({ ...editData, date_sold: e.target.value })} /> : <span style={{ fontSize: 12 }}>{item.date_sold ? formatShortDate(item.date_sold) : "—"}</span>}</td>
